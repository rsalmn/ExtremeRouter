...
// (prevents EBUSY on `npm i -g @rsalmn/extremerouter@latest` while MITM is running).
...
const ENCRYPT_SALT = "extremerouter-mitm-pwd";
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
    ...
    if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
      throw new Error(`MITM server.js not found at ${effectiveServerPath}. Reinstall extremerouter.`);
    }
  }
