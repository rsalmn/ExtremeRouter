...
function RouterNode({ data }) {
  return (
    <div className="flex items-center justify-center px-5 py-3 rounded-xl border-2 border-primary bg-primary/5 shadow-md min-w-[130px]">
      ...
      <img src="/favicon.svg" alt="ExtremeRouter" className="w-6 h-6 mr-2" />
      <span className="text-sm font-bold text-primary">ExtremeRouter</span>
      {data.activeCount > 0 && (
...
export default function ProviderTopology({ providers = [], activeRequests = [], lastProvider = "", errorProvider = "" }) {
  ...
  return (
    <div ref={containerRef} className="h-[320px] w-full min-w-0 overflow-hidden rounded-lg bg-surface-2/50 sm:h-[480px]">
      {providers.length === 0 ? (
