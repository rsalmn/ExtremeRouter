...
export default function CombosPage() {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6">
      ...
      />
      {/* Strategy help */}
      <div className="rounded-panel border border-border-subtle bg-panel/60 p-4 text-xs text-text-muted">
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <div><span className="font-semibold text-text-main">Fallback</span> — tries models in order (next on failure)</div>
          <div><span className="font-semibold text-text-main">Round Robin</span> — rotates models across requests to spread load</div>
          <div><span className="font-semibold text-text-main">Fusion</span> — parallel panel + judge synthesis (N+1 calls)</div>
