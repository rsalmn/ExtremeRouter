...
export default function ProvidersPage() {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6">
      <PageHeader
        title="Providers"
        description="Provider registry and account health"
        icon="dns"
        actions={
          <>
      ...
      />
      {!hasAnyResult && (
        <div className="rounded-panel border border-dashed border-border py-10 text-center">
          <span className="material-symbols-outlined mb-2 text-[28px] text-text-muted">search_off</span>
          <p className="text-sm text-text-muted">No providers match your search</p>
        </div>
      )}
...
function ProviderCard({ providerId, provider, stats, authType, onToggle }) {
  ...
  return (
    <Link href={`/dashboard/providers/${providerId}`} className="group min-w-0">
      <Card
        padding="xs"
        className={`h-full hover:bg-surface-2 dark:hover:bg-white/[0.01] transition-colors cursor-pointer ${allDisabled ? "opacity-50" : ""}`}
      >
    ...
    <div className="flex min-w-0 flex-col gap-3">
      {summary && (
        <div className="flex flex-wrap items-center gap-2 text-xs mb-1 sm:gap-3">
          ...
          {summary.failed > 0 && (
            <span className="px-2 py-0.5 rounded bg-red-500/15 text-danger font-medium">
