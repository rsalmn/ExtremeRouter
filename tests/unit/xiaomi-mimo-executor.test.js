...
describe("xiaomi-mimo executor", () => {
  ...
  it("authenticates Preview calls with the account cookie", () => {
    // 3rd arg is the model (BaseExecutor convention).
    const headers = ex.buildHeaders({ [COOKIE_KEY]: "serviceToken=abc", accessToken: "sk-x" }, true, "mimo-x-pro-preview");
    expect(headers.Cookie).toBe("serviceToken=abc");
    expect(headers.Authorization).toBeUndefined();
  });

  it("authenticates cloud calls with the bearer key", () => {
    const headers = ex.buildHeaders({ accessToken: "sk-x" }, true, "mimo-v2.5-pro");
