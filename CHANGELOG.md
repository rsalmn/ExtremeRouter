...
## Features
- **Video generation: OpenRouter + Vertex AI (Veo)** — new adapter layer under `handlers/videoProviders/` so `/v1/video/generations` can target OpenRouter or Google Cloud credentials. Providers without an adapter keep the previous behaviour (raw body to `{baseUrl}/{action}`, poll `{baseUrl}/{id}`, verbatim passthrough), so the xAI path is unchanged. OpenRouter uses the same async job shape as xAI, POSTs to the `/videos` collection root (no `/generations` suffix), and applies the registry `HTTP-Referer` / `X-Title` headers. Vertex does two-way translation — create → `:predictLongRunning` `{instances[], parameters{}}`, poll → `:fetchPredictOperation` — because Veo does not speak the OpenAI-ish videos shape; access tokens are minted from Service Account JSON via `refreshVertexToken`, and raw API keys are rejected before any billable job is created. Registry gains `videoConfig`, the `video` serviceKind, and video-kind models (Veo 3.1 / 3 / 2, Sora 2 Pro, Seedance 2.0). [`6e9179d`]
- **Video generation: Agnes AI (API)** — adapter for `agnes-video-v2.0` (`POST /v1/videos`, poll `GET /agnesapi?video_id=`) plus `agnes-video-2.5` / `agnes-video-2.5-flash` (`mode`/`seconds`/`size`/`aspect_ratio` contract, Flash locked to 720P with ≤5 images / ≤3 audios / no videos). Playground aliases (`duration`, `resolution`, `aspect_ratio`) map onto the documented fields; raw documented fields always win. `num_frames` is snapped to the `8n+1` rule so duration×fps never produces a value upstream rejects. [`94f2359`]
- **Xiaomi MiMo Desktop dual auth** — MiMo Desktop support merged into the existing `xiaomi-mimo` provider as dual auth instead of a separate provider, so the dashboard shows one MiMo entry. API key (`sk-`) covers the cloud API; Desktop/OAuth adds the account session used by the two Desktop-exclusive Preview models (`mimo-x-pro-preview`, `mimo-x-flash-preview`), which are served by the account-service route on `mimo-server-sgp.xiaomimimo.com` and authorized by a Xiaomi account session cookie. Custom ECDH encrypted-callback OAuth flow (X25519 → SHA256 → AES-256-GCM) with a loopback callback proxy, one-click import of the local Desktop cookie store, and weekly quota from the account session. [`86ec1fd`, `1310d01`, `8a18f5a`, `31d4589`, `4b85a93`, `f038dbe`]
- **OpenCode Go catalog** — models the provider docs list but the registry lacked: `glm-5.3`, `kimi-k3`, `deepseek-v4.1-flash`, `longcat-2.0`, `hy4-preview`, `hy3` on chat/completions; `qwen3.8-max` + `qwen3.8-flash` on `/messages`; `grok-4.6` + `gpt-5.6-luna` responses-only. Chat-only models stay on the sourceFormat-matched transport so a Claude client is never routed to `/messages` for a model that lacks it. Secondary alias `ocg` now resolves for targetFormat lookup. [`5a955a1`]
- **Text-to-Video form controls** — the video playground card now exposes Duration (s), Aspect Ratio, Ratio (Runway/Bynara), and Resolution, filtered by each model's `params` list. Selects support a Custom option for free-form values; empty fields are omitted so provider defaults still apply. [`099a568`]

## Fixes
- **Vertex video path traversal** — model ids and operation names are interpolated into the Vertex URL unescaped. A crafted segment (e.g. `../../evil`, or an operation name with `..` in a field) could splice a path traversal into the fetch URL while the Bearer token stayed attached. Model ids are now restricted to `[A-Za-z0-9._-]` (never `.` / `..`, at most one `provider/` prefix); operation names must match the anchored `projects/…/operations/` shape with single-segment fields; `project_id` / `location` get the same charset guard. Rejected before any fetch. [`65e7a25`]
- **CodeBuddy CN: deepseek-v4-flash → deepseek-v4.1-flash** — the product-config payload from `copilot.tencent.com` no longer lists `deepseek-v4-flash`; `deepseek-v4.1-flash` is the published contract id. `maxOutput` rises 50000 → 128000 (`contextWindow` stays 1M). Effort tiers are `low`/`high`/`xhigh` on the CodeBuddy CN gateway. [`0a3867f`]
- **NaraRouter removed; Bynara OAuth unknown-provider crash** — NaraRouter (`nara`) was a thin alias of `router.bynara.id` already covered by the real Bynara provider; registry entry, icon, and gate pins dropped. Bynara is `category: free` + API-key only, so the provider page treated it as OAuth (`FREE_PROVIDERS` ⊂ `isOAuth`) and `GET /api/oauth/bynara/authorize` threw "Unknown provider: bynara". Declare `hasOAuth: false` + `authModes: apikey`, and propagate an explicit `hasOAuth: false` through `buildProviderEntry` (it previously only forwarded `true`). Free-provider stats now query `apikey` for `hasOAuth: false` entries. [`03c6c1c`]
- **Agnes video: poll rate limits and duration window** — the status endpoint rate-limits (~6 queries per short window, "too many video status queries") and the 1.5s poll tripped it immediately, so the loop spent its budget on 429s and timed out while the render finished upstream and was billed Success. Poll interval raised to 5s with ≥10s backoff (doubling to 30s); rate-limit errors surfaced inside an HTTP 200 envelope are detected; the legacy `GET /v1/videos/{task_id}` endpoint unlocks after repeated limits or 8 pending polls; the 10-minute video poll timeout replaced the shared 2-minute image window. Separately, Agnes 2.5/2.5-flash accept `seconds` only as `"4"`–`"12"`, but the playground Duration field allows up to 600s — out-of-range values are now clamped into the window instead of returning 400 before a job is created. [`ae02457`, `4d50143`]

## Tests
- New suites: `openrouter-vertex-video` (15), `agnes-api-video-provider` (23), `video-form-fields` (5), `bynara-oauth-gating` (5), `xiaomi-mimo-executor` (10), `xiaomi-mimo-oauth-proxy` (4), `xiaomi-mimo-oauth-session` (6). `opencode-go-models` re-enabled after the catalog matched. Vertex path-traversal, Agnes 8n+1 snap / 429 backoff / 4-12s clamp, and the poll-status/exchange session lifecycle are pinned as regressions.
